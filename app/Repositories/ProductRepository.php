...
class ProductRepository extends BaseRepository
{
    ...
    public function storeProduct($input)
    {
        try {
            ...
            $product->order_create_by = auth()->user()->id;
//            $product->save();
            ...
            //for create product addon / variants

                $addonItemIds = json_decode($input['addon_data'], true);
                if ($input['is_addon_check'] == 1){
                    foreach ($addonItemIds as $item){
                        ProductVariation::create([
                            'product_id' => $product->id,
                            'addons_id' => $item['addon_id'],
                            'addon_items_id' => $item['addon_item_id'],
                            'price' => $item['price'],
                        ]);
                    }
                }
...
//            $product->save();
            ...
            $addonItemIds = json_decode($input['addon_data'], true);
            if ($input['is_addon_check'] == 1){
               $getOldData = ProductVariation::where('product_id', $product->id)->delete();
                foreach ($addonItemIds as $item){
                    ProductVariation::create([
                        'product_id' => $product->id,
                        'addons_id' => $item['addon_id'],
