...
class SaleRepository extends BaseRepository
{
    ...
    public function storeSale($input): Sale
    {
        try {
            ...
            $input['status'] = Sale::RECEIVE; // set status to RECEIVE
            $input['is_sale_created'] = $input['is_sale_created'] ?? false;
            $QuotationId = $input['quotation_id'] ?? false;
            $saleInputArray = Arr::only($input, [
                'customer_id', 'warehouse_id', 'tax_rate','order_type', 'tax_amount', 'discount', 'shipping', 'grand_total',
            ...
            $sale->accept_timer = $sale->created_at->addMinutes(2); // add 2 minutes timer for processing
            $sale->save();
...
//            $url = "https://xrjv8e.api.infobip.com/sms/2/text/advanced";
    ...
    public function calculationSaleItems($saleItem)
    {
        ...
        $saleItem['sub_total'] = ($saleItem['net_unit_price'] + $perItemTaxAmount ) + ($saleItem['addons_price'] ?? 0) * $saleItem['quantity'];
    ...
    {
        foreach ($input['sale_items'] as $saleItem) {
            $saleItem = Arr::except($saleItem, [
                'addons', 'addon_item_id_array',
            ]);

            if (isset($saleItem['addons_value'])){
                $saleItem['addons_value'] = json_encode($saleItem['addons_value']);
            }
