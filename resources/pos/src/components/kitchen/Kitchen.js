...
const Kitchen = (props) => {
    ...
    const itemsValue = currencySymbol && sales.length >= 0 && sales.map(sale => ({
        ...
        order_type: sale.attributes.order_type,
            ...
            cell: row => {
                return (
                    ...
                    <span className='badge bg-light-info'>
                        <span>{getFormattedMessage("order.status.receive.label")}</span>
                    </span> ||
                    row.status === 5 &&
                    <span className='badge bg-light-info'>
                        <span>{getFormattedMessage("order.status.on-process.label")}</span>
                    </span> ||
                    row.status === 6 &&
                    <span className='badge bg-light-success'>
                        <span>{getFormattedMessage("order.status.order-ready.label")}</span>
                    </span> ||
                    row.status === 7 &&
                    <span className='badge bg-light-info'>
                        <span>{getFormattedMessage("order.status.delivered.label")}</span>
                    </span>
                )
            }
        },
        {
            name: getFormattedMessage('pos-order-type.title'),
            selector: row =>  row.order_type.toUpperCase(),
...
export default connect(mapStateToProps, { fetchSales, salePdfAction, fetchFrontSetting })(Kitchen);
