...
const ProductForm = (props) => {
   ...
   const addonItemPriceHandler =  (e) => {
    for (let i = 0; i < selectedOptions.length; i++) {
        if (selectedOptions[i]['label'] === e.target.name) {
            selectedOptions[i].price = e.target.value;
          break; // exit the loop if the object is found
        }
      }
      setProductValue({ ...productValue, addon_data: JSON.stringify(selectedOptions) });

   }
    const handleSelectChange = (selected) => {
        setSelectedOptions(selected);
        setProductValue({ ...productValue, addon_data: JSON.stringify(selected) });
    };

    const isAddonCheckHandler = (e) => {
        setIsAddonChecked(e.target.checked ? 1 : 0);
        setProductValue({ ...productValue, is_addon_check: e.target.checked ? 1 : 0  });
        if (!e.target.checked)  setSelectedOptions([]);
    }

    function checkedAddonCheckBox(){
        if(singleProduct) {

            if (singleProduct[0].product_addons.length > 0) {
                setIsAddonChecked(1);
                setProductValue({ ...productValue, is_addon_check: 1 });
    ...
    function fetchAddons() {
        let AddonsGroupAll = [];
        apiConfig.get('/addons')
            .then((response) => {
                const AddonsGroup = response.data.data.map((addon) => {
                    let addonItems = addon.attributes.addon_items.map((addonItem) => {
                        return {
                            value: addonItem.id,
                            label: addonItem.name,
                            addon_id:addon.id,
                            addon_item_id:addonItem.id,
                            price:1,
                            addon_name:addon.attributes.name
                        }
                    })
                    return {
                        label: addon.attributes.name,
                        options: addonItems,
                    }
                })

                AddonsGroupAll.push(...AddonsGroup);
            })
            .catch(({ response }) => {
            });
        setGroupOptions(AddonsGroupAll);
    };
    ...
    function mapAddons(){
            ...
            const AddonsGroup = singleProduct[0].product_addons.map((addon) => {
                    let addonItems = addon.addon_items.map((addonItem) => {
                        return {
                            ...
                            addon_item_id:addonItem.item_id,
                            price:addonItem.item_price,
        ...
        setSelectedOptions(AddonsGroup.flat());
        return AddonsGroup.flat();
        }
    ...
    useEffect(() => {
        ...
        if (singleProduct) {
            setProductValue({
                ...
                making_time: singleProduct ? singleProduct[0].making_time ? singleProduct[0].making_time : "" : "",
                ...
                // addon_data: singleProduct ? singleProduct[0].addon_data : [],
                addon_data: singleProduct ? singleProduct[0].product_addons : [],
                is_addon_check: singleProduct ? 1 : 0,
            })
        }
    }, []);
    ...
    const prepareFormData = (data) => {
        ...
        formData.append('making_time', data.making_time ? data.making_time : "");
        formData.append('is_addon_check', data.is_addon_check ? data.is_addon_check : 0);
        formData.append('addon_data', data.addon_data ? data.addon_data : "");
        if (data.tax_type[0]) {
                        ...
                        <div className='col-xl-8'>
                            <div className='card'>
                                <div className='row'>
                                    ...
                                    <div className='col-md-6 mb-3'>
                                        <label
                                            className='form-label'>{getFormattedMessage('product.input.making-time.label')}: </label>
                                        <span className='required' />
                                        <input type='number' name='making_time'
                                            className='form-control'
                                            placeholder={placeholderText('product.input.making-time.placeholder')}
                                            onKeyPress={(event) => decimalValidate(event)}
                                            onChange={(e) => onChangeInput(e)}
                                            value={productValue.making_time} min={1} />
                                        <span
                                            className='text-danger d-block fw-400 fs-small mt-2'>{errors['making_time'] ? errors['making_time'] : null}</span>
                                    </div>
                                        ...
                                        <Form.Check
                                            ...
                                            id='is_addon_check'
                                            label={`Have Addons`}
                                            onChange={isAddonCheckHandler}
                                            checked={isAddonChecked === 1 ? true : false}
                                        />
                                        </div>
                                        {isAddonChecked == 1 &&
                                            <div className='col-md-12 mb-3'>
                                                <ReactMultiSelect title="Addon/Customization"
                                    ...
                                    {isAddonChecked == 1 && selectedOptions.length > 0 && selectedOptions.map((selectedOption,index) => {
                                        return (
                                            <>
                                                <div key={index} className='col-md-3 mb-3'>
                                                    <p className='small'>{selectedOption.addon_name}</p>
                                                    <p>{selectedOption.label} Price</p>
                                                    <input type='number' name={selectedOption.label}
                                                        className='form-control'
                                                        min={1}
                                                        onChange={addonItemPriceHandler}
                                                        defaultValue={selectedOption.price ? selectedOption.price : 1 }
                                                           value={selectedOption.price ?? selectedOption.price }
                                                        />
                                                </div >
                                            </>
                                        )
