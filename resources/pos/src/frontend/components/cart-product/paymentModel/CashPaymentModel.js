...
const CashPaymentModel = (props) => {
    const { handleCashPayment, cashPaymentValue, onPaymentStatusChange, cashPayment, onChangeInput, onCashPayment, grandTotal, totalQty, cartItemValue, taxTotal, settings, subTotal, errors, onPaymentTypeChange, paymentTypeDefaultValue, paymentTypeFilterOptions, allConfigData, onChangeReturnChange,orderType } = props;
    ...
    return (
        <Modal show={cashPayment} onHide={handleCashPayment} size='lg' className="pos-modal">
            <Modal.Header closeButton>
                <Modal.Title>{getFormattedMessage('pos-order-confirmation.title')} | {getFormattedMessage('pos-order-type.title')} :  {orderType.toUpperCase()} </Modal.Title>
            </Modal.Header>
            ...
            <Modal.Footer className="mt-0">
                <button type='button' className='btn btn-primary' onClick={(event) => {
                    ...
                    {getFormattedMessage("globally.checkout-btn")}
                </button>
